...
export default class MapView implements MapViewControls, TileDataSource {
  ...
  pickTile(worldPos: Vector3): TileData | null {
    ...
    const q = (1.0 / 3 * Math.sqrt(3) * x - 1.0 / 3 * y);
    const r = 2.0 / 3 * y;

        // now need to round the fractal axial coords into integer axial coords for the grid lookup
    const cubePos = axialToCube(q, r);
    const roundedCubePos = roundToHex(cubePos);
    const roundedAxialPos = cubeToAxial(roundedCubePos.x, roundedCubePos.y, roundedCubePos.z);
