...
function randomHeight(q: number, r: number) {
  const noise1 = simplex2(q / 10, r / 10);
  const noise2 = perlin2(q / 5, r / 5);
  const noise3 = perlin2(q / 30, r / 30);
...
function isAccessibleMountain(tile: TileData, grid: Grid<TileData>) {
  const ns = grid.neighbors(tile.q, tile.r);
  const spring = isMountain(tile.height);
