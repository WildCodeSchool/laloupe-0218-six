...
export function createHexagon(radius: number, numSubdivisions: number): BufferGeometry {
  ...
  let border = new Float32Array(numFaces * 3 * 1), e = 0;
  ...
  let vertices: Vector3[] = []; // every three vertices constitute one face
...
export function randomPointInHexagon(hexRadius: number): Vector3 {
  ...
  const A = computeHexagonCorner(hexRadius, ((startCornerIndex + 0) % 6) / 6.0);
  const B = new Vector3(0, 0, 0);
  const C = computeHexagonCorner(hexRadius, ((startCornerIndex + 1) % 6) / 6.0);

  // random point in the triangle based on AB and AC
  const r = Math.random(), s = Math.random();
  const rSqrt = Math.sqrt(r), sSqrt = Math.sqrt(s);
